...
import type { DeployStatus } from "../worktree/types.js";
 ...
 */
function parseDeploysOutput(output: string): DeployEntry[] {
  ...
  const dataLines = lines.filter(line => {
    ...
    if (line.startsWith("The --detail option")) return false;
    if (line.startsWith("service ")) return false;
    return true;
  });
...
async function isAncestor(ancestor: string, descendant: string): Promise<boolean> {
  try {
    await execAsync(
      `git -C "${BENTO_DIR}" merge-base --is-ancestor ${ancestor} ${descendant}`,
    );
    return true;
  } catch {
 ...
 */
function mapDeployStatus(iscStatus: string): DeployStatus {
  switch (iscStatus) {
    case "pending":
      return "pending";
    case "in-progress":
...
export async function fetchDeployStatus(
  ...
  try {
    const { stdout } = await execAsync(
      `isc deploys -e production ${service} --count 20`,
    );
