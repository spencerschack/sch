...
export async function listWindows(): Promise<CursorWindow[]> {
...
export async function minimizeWindow(pattern: string): Promise<string[]> {
  const script = `
...
export async function restoreWindow(pattern: string): Promise<string[]> {
  const script = `
tell application "System Events"
    tell process "Cursor"
        set restored to {}
...
export async function focusWindow(pattern: string): Promise<string | null> {
  const script = `
tell application "System Events"
    tell process "Cursor"
        repeat with w in windows
