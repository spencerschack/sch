 ...
 */
export async function getWorktreeWorkingDir(worktreeName) {
 ...
 */
export async function getAgentProvider(worktreeName) {
    const config = await readWorktreeConfig(worktreeName);
 ...
 */
export async function launchAgent(worktreeName) {
    ...
    switch (provider) {
        case "cursor":
            ...
            break;
        case "claude":
            await execAsync(`tmux new-session -Ads "${worktreeName}" -c "${workingDir}" "claude --dangerously-skip-permissions"`);
            break;
        case "cursor-cli":
            await execAsync(`tmux new-session -Ads "${worktreeName}" -c "${workingDir}" "cursor agent chat"`);
            break;
    }
}
/**
 ...
 */
export async function openAgent(worktreeName) {
    const provider = await getAgentProvider(worktreeName);
    const workingDir = await getWorktreeWorkingDir(worktreeName);
    switch (provider) {
        case "cursor": {
